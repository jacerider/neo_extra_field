 ...
 * @package Drupal\neo_extra_field\Plugin
 */
class NeoExtraFieldDisplayManager extends NeoExtraFieldManagerBase implements NeoExtraFieldDisplayManagerInterface {

  /**
   * The plugin's subdirectory.
   */
  const PLUGIN_SUBDIR = 'Plugin/NeoExtraField/Display';

  /**
   * The interface each plugin should implement.
   */
  const PLUGIN_INTERFACE = 'Drupal\neo_extra_field\Plugin\NeoExtraFieldDisplayInterface';

  /**
   * The name of the annotation that contains the plugin definition.
   */
  const PLUGIN_ANNOTATION_NAME = 'Drupal\neo_extra_field\Annotation\NeoExtraFieldDisplay';

  /**
   * Name of the alter hook for the plugins.
   */
  const ALTER_HOOK = 'neo_extra_field_display_info';

  /**
   * Name of the cache key for plugin data.
   */
  const CACHE_KEY = 'neo_extra_field_display_plugins';
  ...
  /**
   * Constructor for NeoExtraFieldManager objects.
   *
  ...
  public function entityView(array &$build, ContentEntityInterface $entity, EntityViewDisplayInterface $display, $viewMode) {
    ...
    foreach ($definitions as $pluginId => $definition) {
      ...
      $component = $display->getComponent($this->fieldName($pluginId));
      if (!$component) {
        continue;
      }
      /** @var NeoExtraFieldDisplayInterface $plugin */
